...
export default function Sidebar({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  ...
  const MENU_LINKS = [
    ...
    { name: 'ID Card', icon: Contact, onPress: () => { onClose(); router.push('/screens/id-card'); } },
    ...
    { name: 'My Profile', icon: User, onPress: () => { router.push('/screens/profile'); setTimeout(() => { onClose(); }, 50); } },
