...
export default function NotificationHistoryScreen() {
  ...
  const fetchNotifications = useCallback(async (isRefresh = false) => {
    try {
      ...
      // Display from index 25 onwards
      setNotifications(employeeNotifs.slice(30));
    ...
    <View className="flex-1 bg-[#f5f7f9]">
      ...
      <SafeAreaView edges={['top']} style={{ backgroundColor: '#ffffff', paddingTop: Platform.OS === 'android' ? StatusBar.currentHeight : 0, zIndex: 50 }}>
        <View style={{
            ...
            {/* Left: Back Arrow */}
            <TouchableOpacity onPress={() => router.canGoBack() ? router.back() : router.replace('/screens/notification')} className="flex-row items-center">
              ...
              if (filteredNotifications.length === 0) {
                return (
                  <View className="flex-1 justify-center items-center py-20 mt-10">
                    ...
                    <Text className="text-[20px] font-bold text-[#011023] uppercase tracking-wide mb-2">No history found</Text>
