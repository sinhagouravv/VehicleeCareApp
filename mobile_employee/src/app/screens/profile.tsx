              ...
              <View style={{ gap: 5, paddingHorizontal:17, paddingVertical:11 }}>
                ...
                <View className="flex-row items-center justify-between">
                  ...
                  <Text numberOfLines={1} style={{ maxWidth: '60%' }} className="text-[#011023] font-semibold uppercase text-right">
                    ...
                    })() : 'N/A'} <Text style={{ marginHorizontal: 3, transform: [{ translateY: -1 }] }} className="text-[#011023] text-[13px] font-semibold">|</Text> {user?.createdAt ? new Date(user.createdAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(/:/g, ':') : 'N/A'}
                  </Text>
                </View>
                <View className="flex-row items-center justify-between">
                  <Text className="text-slate-700 uppercase font-semibold pr-1">Address</Text>
